...
if (mysqli_num_rows($query2)){
	...
	$query3 = mysqli_query($con, "INSERT INTO subscribers_acct(email, password, acct_type,firstname) VALUES ('$email','$pass','Gold subscriptionUser','$name')");
	...
	//$query2 = mysqli_query($con,"INSERT INTO Payments (email,reference_id,message,status,transaction_id,date,time,amount) VALUES ('$email','$reference','$message','$status','$transaction_id','$date','$time','$amount')");
		...
		if($query){
		    ...
		    echo "<script language='javascript' type='text/javascript'> location.href='gold_pay.php?nsa=$order_id' </script>";
  ...
  if ($query && !empty($_POST)){
        ...
        $to = $email;
        $subject = "SmoothRepairs Subscription Order for $first_name $last_name";

        $message2 = "
        <html>
        <head>
        <title>SmoothRepair Subscription Order Notice</title>
        </head>
        <body>
        <p>Dear $Fname,</p>
        <p>Thanks for choosing <strong>SmoothRepairs</strong>. <br>Your <strong>Subscription</strong> order has been received and you will be notified by our technical team within 24Hrs.</p>
        ...
        $subject2 = "SmoothTracker Subscription Order for $first_name $last_name";

        $message = "
        <html>
        <head>
        <title>SmoothRepairs Subscription Order Notice</title>
        </head>
        <body>
