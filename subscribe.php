                    ...
                    <form method="POST" action="" name="subscription" enctype="multipart/form-data">
                        ...
                        <div class="col-sm-6">
                          ...
                          <div class="col-sm-6">
                            ...
                            <button class="btn  btn-lg btn-primary btn-block " name="subscribe" type="submit">Continue Subscription</button>
...
if (mysqli_num_rows($query2)){
	...
	$query3 = mysqli_query($con, "INSERT INTO subscribers_acct(email, password, acct_type,firstname) VALUES ('$email','$pass','Bronze subscriptionUser','$name')");
	...
	//$query2 = mysqli_query($con,"INSERT INTO Payments (email,reference_id,message,status,transaction_id,date,time,amount) VALUES ('$email','$reference','$message','$status','$transaction_id','$date','$time','$amount')");
		...
		if($query){
		    ...
		    $last_id = mysqli_insert_id($con);
                    $order_id = $last_id;
				echo "<script type='text/javascript'>alert('Your SmoothRepairs Bronze Plan Subscription has been submitted successfully')</script>";
				
                
                echo "<script language='javascript' type='text/javascript'> location.href='bronze_pay.php?nsa=$order_id' </script>";
  ...
  if ($query && !empty($_POST)){
        ...
        $to = $email;
        $subject = "SmoothRepairs Bronze Plan Subscription Order for $first_name $last_name";

        $message2 = "
        <html>
        <head>
        <title>SmoothRepair Bronze Plan Subscription Order Notice</title>
        </head>
        <body>
        <p>Dear $Fname,</p>
        <p>Thanks for choosing <strong>SmoothRepairs</strong>. <br>Your <strong>Bronze Plan Subscription</strong> order has been received and you will be notified by our technical team within 24Hrs.</p>
        ...
        <p>A new Bronze Plan Subscription order has been received for <strong>$first_name $last_name</strong>.</p>
